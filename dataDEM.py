...
class DeepEnergyMethod:
    ...
    def get_data_loss(self, data_points, data_values): # calculates loss from data 
#         return torch.nn.functional.mse_loss(self.getU(data_points), data_values, reduction = "sum")
        return torch.nn.functional.mse_loss(self.getU(data_points) + data_points, data_values, reduction = "sum")
    
    def get_PDE_loss(self, shape, dxdydz, data, neumannBC, dirichletBC): # calculates loss from physical model
...
#             dirBC_penalty[i] = torch.tensor(dirichletBC[keyi]['penalty']).float().to(dev)
        ...
        # ----------------------------------------------------------------------------------
        # Loss function (energy and boundary conditions)
        # ----------------------------------------------------------------------------------
        # ----------------------------------------------------------------------------------
        # Internal Energy
        # ----------------------------------------------------------------------------------
        ...
        # Energy from traction on the boudary
        ...
        # Dirichlet boundary (seems redundant to my problem)
...
#         boundary_loss = torch.sum(bc_u_crit)
        loss = (internal2 - torch.sum(external2))/Height*Length*Depth #+ boundary_loss
        return loss, energy_loss#, boundary_loss
    
    def train_model(self, shape, dxdydz, data, neumannBC, iteration, learning_rate, rho, data_ds, data_values_ds):
        x = torch.from_numpy(data).float()
        x = x.to(dev)
        x.requires_grad_(True)
        # get tensor inputs and outputs for boundary conditions
...
#             dirBC_penalty[i] = torch.tensor(dirichletBC[keyi]['penalty']).float().to(dev)
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                u_pred = self.getU(x)
                u_pred.double()
                storedEnergy, detF = self.energy.getStoredEnergy(u_pred, x)
                # Energy density
                internal2 = self.intLoss.lossInternalEnergy(storedEnergy, dx=dxdydz[0], dy=dxdydz[1], dz=dxdydz[2], shape=shape)
                # Energy from traction on the boudary
                external2 = torch.zeros(len(neuBC_coordinates))
...
#                     bc_u_crit[i] = self.loss_squared_sum(dir_u_pred, dirBC_values[i]) * dirBC_penalty[i]
#                 energy_loss = internal2 - torch.sum(external2)
                energy_loss = (internal2 - torch.sum(external2))/Height*Length*Depth
                data_loss = self.get_data_loss(data_ds, data_values_ds)/data_ds.shape[0]
                det_loss = torch.max(torch.nn.functional.relu(-detF))#/x.shape[0]
#                 boundary_loss = torch.sum(bc_u_crit)
#                 loss = internal2 - torch.sum(external2) + boundary_loss                
                loss = (1 - rho)*data_loss + rho*energy_loss 
                optimizer.zero_grad()
                loss.backward()
                print('Iter: %d Loss: %.9e Energy: %.9e Data: %.9e Det: %.9e Time: %.3e'
                      % (t + 1, loss.item(), energy_loss.item(), data_loss.item(), det_loss.item(), time.time() - it_time))
...
#                 boundary_loss_array.append(boundary_loss.item())
                data_loss_array.append(data_loss.item())
                det_loss_array.append(det_loss.item())
                loss_array.append(loss.item())
                return loss
            optimizer.step(closure)
            u_array.append(self.getU(x))
        elapsed = time.time() - start_time
#         print('Training time: %.4f' % elapsed)
        return loss_array, energy_loss_array, data_loss_array, det_loss_array, u_array#boundary_loss_array
    
    # Fourier filter
    def Fourier(self, p, N_Fourier, use_Fourier = True):
        if use_Fourier == True:
            sigma = 0.1
            np.random.seed(0)
        #     h_m = torch.Tensor(np.random.multivariate_normal(mean = [0, 0, 0], cov = [[sigma**2, 0, 0], [0, sigma**2, 0], [0, 0, sigma**2]], size = N_Fourier))
            h_m = torch.Tensor(np.random.normal(loc = 0, scale = sigma, size = (N_Fourier, 3)))
            if p.dim() == 1:
                p_h_m = torch.unsqueeze(p, 0) @ h_m.T
                return torch.cat((torch.cos(2*np.pi*p_h_m), torch.sin(2*np.pi*p_h_m)), 1).flatten()
            else:
                p_h_m = p @ h_m.T
                return torch.cat((torch.cos(2*np.pi*p_h_m), torch.sin(2*np.pi*p_h_m)), 1)
        else:
            return p

    def getU(self, x): # displacement u
        # Sanity check analytical solution
#         k = 1

#         ux = torch.zeros(x.shape[0])
#         uy = x[:,1]*(torch.cos(torch.pi*k/Length*x[:,0]) - 1) - x[:,2]*torch.sin(torch.pi*k/Length*x[:,0])
#         uz = x[:,1]*torch.sin(torch.pi*k/Length*x[:,0]) + x[:,2]*(torch.cos(torch.pi*k/Length*x[:,0]) - 1)

#         u_test = torch.cat((torch.unsqueeze(ux,0), torch.unsqueeze(uy,0), torch.unsqueeze(uz,0)),0).T

        x = self.Fourier(x, N_Fourier, use_Fourier)
        u = self.model(x)
        Ux = x[:, 0] * u[:, 0]
        Uy = x[:, 0] * u[:, 1]
        Uz = x[:, 0] * u[:, 2]
...
#         return loss
# ==========================================================================================================================================
# GETTING DATA (FEM SOLUTION + NOISE)
# ==========================================================================================================================================
Length = 4.0
Height = 1.0
Depth = 1.0
...
# FEM solution
u_FEM = np.loadtxt("values_FEM.csv", delimiter = ",") # FEM solution
...
# Ny_down = 6 #4 # Number of downsampled points in the y direction
# Nz_down = 6 #4 # Number of downsampled points in the z direction

# ind = np.lexsort((vertices_FEM[:,2],vertices_FEM[:,1], vertices_FEM[:,0]))

# vertices_FEM = vertices_FEM[ind].reshape(41, 11, 11, 3)
# vertices_FEM = vertices_FEM[np.linspace(0, vertices_FEM.shape[0] - 1, Nx_down, dtype = int), :, :, :]
# vertices_FEM = vertices_FEM[:, np.linspace(0, vertices_FEM.shape[1] - 1, Ny_down, dtype = int), :, :]
# vertices_FEM = vertices_FEM[:, :, np.linspace(0, vertices_FEM.shape[2] - 1, Nz_down, dtype = int), :]
# vertices_FEM = vertices_FEM.reshape(vertices_FEM.shape[0]*vertices_FEM.shape[1]*vertices_FEM.shape[2], 3)

# u_FEM = u_FEM[ind].reshape(41, 11, 11, 3)
# u_FEM = u_FEM[np.linspace(0, u_FEM.shape[0] - 1, Nx_down, dtype = int), :, :, :]
# u_FEM = u_FEM[:, np.linspace(0, u_FEM.shape[1] - 1, Ny_down, dtype = int), :, :]
# u_FEM = u_FEM[:, :, np.linspace(0, u_FEM.shape[2] - 1, Nz_down, dtype = int), :]
# u_FEM = u_FEM.reshape(u_FEM.shape[0]*u_FEM.shape[1]*u_FEM.shape[2], 3)

# np.savetxt("values_FEM_downsampled3.csv", u_FEM, delimiter = ",")
# np.savetxt("vertices_FEM_downsampled3.csv", vertices_FEM, delimiter = ",")

# # Sanity check: using a known deformation profile to test if calculations are correct
# k = 1

# x = torch.Tensor(vertices_FEM_ds)
...
# np.random.seed(10)
# data_test = (u_test + x).detach().numpy()
# # data_test[vertices_FEM[:,0] != 0] = (data_test[vertices_FEM[:,0] != 0] - np.min(data_test[vertices_FEM[:,0] != 0],axis=0))/(np.max(data_test[vertices_FEM[:,0] != 0],axis=0) - np.min(data_test[vertices_FEM[:,0] != 0],axis=0))
# data_test[vertices_FEM_ds[:,0] != 0] = data_test[vertices_FEM_ds[:,0] != 0] + np.random.normal(loc = 0.0, scale = 0.01, size = (data_test[vertices_FEM_ds[:,0] != 0].shape[0], 3))
# data_test = torch.Tensor(data_test)

# Adding noise to FEM solution
np.random.seed(10)
# data_values = torch.Tensor(u_FEM + np.random.normal(loc = 0.0, scale = 0.03, size = (u_FEM.shape[0], 3)))
data_values_ds = u_FEM_ds + vertices_FEM_ds
data_values_ds[vertices_FEM_ds[:,0] != 0] = data_values_ds[vertices_FEM_ds[:,0] != 0] + np.random.normal(loc = 0.0, scale = 0.01*(np.max(data_values_ds[vertices_FEM_ds[:,0] != 0]) - np.min(data_values_ds[vertices_FEM_ds[:,0] != 0])), size = (data_values_ds[vertices_FEM_ds[:,0] != 0].shape[0], 3))
data_values_ds = torch.Tensor(data_values_ds)
# ==========================================================================================================================================
# TRAINING PINN
# ==========================================================================================================================================
# %matplotlib inline
...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------------------------
    dom, boundary_neumann = des.setup_domain_from_FEM() # Set up physical and implementation parameters
#     x, y, z, datatest = des.get_datatest()
    # ----------------------------------------------------------------------
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    mat = md.EnergyModel('neohookean', 3, mu, lambd) # set up neo-Hookean energy function
    dem = DeepEnergyMethod([N_input, 100, cf.D_out], 'simpson', mat, 3)
#     Transfer learning
#     dem.model.load_state_dict(dem_init.model.state_dict())
#     ----------------------------------------------------------------------
...
#     cf.filename_out = "./output/dem/NeoHook_3Layer_mesh100x25x25_iter40_trap"
    loss, energy, data, det, u_res = dem.train_model(cf.shape, cf.dxdydz, dom, boundary_neumann, cf.iteration, cf.lr, rho, torch.Tensor(vertices_FEM_ds), data_values_ds)
